...
function_args = {
    name: list(inspect.signature(func).parameters.keys())[1:]  # skip 'driver'
...
def add_queue_slot():
    ...
    dropdown = tk.OptionMenu(frame, func_var, *available_functions.keys(), command=lambda _: update_inputs(func_var, input_frame))
...
def update_inputs(func_var, input_frame):
